...
(function () {
  ...
  function labBend(lab) {
    ...
    var W = 820, H = 360;
    var cx = 410, cy = 180;
    var Y0 = 72;                       /* 入射高度：不受引力时，光就沿这条水平线笔直走 */
    var path = [];
    var dotT = 0, last = 0;

    function computePath(mass) {
      path = [];
      var x = -20, y = Y0, vx = 260, vy = 0;
      /* 引力耦合系数由 1500 降到 700：原值下质量 ≥24 时光线就冲出画布底边
         （mass=50 时轨迹最低点 y≈1074，是画布高的 3 倍，学生只看到"掉下去了"）。
         700 经轨迹实测：质量 1→50 全程都从右边离开画布（最低点 336 < 360），
         偏折角从 0.6°（1 倍太阳，几乎笔直）平滑增到 33°（50 倍太阳，一眼看得出弯）。 */
      var k = 700 * mass;
      var dt = 0.016;
      for (var i = 0; i < 420; i++) {
    ...
    function draw(ts) {
      ...
      /* 对照线：如果不受引力，光会一直沿着这条水平线笔直走出去。
         有了它，光被"掰"下去多少，一眼可比。 */
      ctx.strokeStyle = "#9AA7BE"; ctx.lineWidth = 1.6; ctx.setLineDash([7, 7]);
      ctx.beginPath(); ctx.moveTo(0, Y0); ctx.lineTo(W, Y0); ctx.stroke();
      ctx.setLineDash([]);
      ctx.fillStyle = "#5c6b82"; ctx.font = "600 12px -apple-system, sans-serif";
      ctx.fillText("灰色虚线：没有引力时，光本该笔直走", 26, Y0 - 10);
      ...
      var exitDeg = 0;
      if (path.length > 4) {
        var q1 = path[path.length - 1], q0 = path[path.length - 4];
        exitDeg = Math.atan2(q1.y - q0.y, q1.x - q0.x) * 180 / Math.PI;
      }

      if (vSpan) vSpan.textContent = mass.toFixed(0) + " M☉";
      if (out) {
        out.innerHTML = "恒星质量 = " + mass.toFixed(0) + " 倍太阳　·　光被掰弯约 <b>" +
          Math.abs(exitDeg).toFixed(1) + "°</b>　·　质量越大，时空弯得越狠，偏折越明显" +
